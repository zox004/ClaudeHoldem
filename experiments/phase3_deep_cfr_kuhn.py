...
def _train_until(
    ...
    # Phase 3 Day 5 H Tier 1 logging — see phase3_deep_cfr_leduc._train_until
    # for full comment. Aggregates 3 events/iter into one wandb.log at
    # step=iter, so the train/* curves merge with checkpoint metrics.
    ...
    for ev in new_events:
        ...
        net = str(ev["net"])
        tag = f"adv_p{int(ev['player'])}" if net == "advantage" else "strat"  # type: ignore[arg-type]
