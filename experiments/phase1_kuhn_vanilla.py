...
Trains :class:`VanillaCFR` on Kuhn for ``cfg.iterations`` cycles, logging
exploitability (chips/g and mbb/g) and P1 game value at a mix of dense-prefix
and sparse-tail cadences. Reaches the Phase 1 Exit Criterion (< 5 mbb/g @ 10k
iter) and records the first iter at which the threshold is crossed.

Structure reuse
---------------
~70% structural reuse from ``experiments.phase1_rps_convergence`` (Week 1):
...
Divergences:
    - Single seed (Vanilla CFR is deterministic — multi-seed would be fake
      variance). Phase 2 MCCFR will re-introduce the 3-seed loop structure
      of the Week 1 RPS harness.
    - Inner loop is ``CFR.train(1 iter)`` called ``iterations`` times so we
      can snapshot exploitability along the way — not a single bulk
      ``train(10000)`` call.
    - Plot is 2-panel (log-log convergence + linear-scale mbb/g with Exit
      Criterion + expected-final reference lines).

Run:
    uv run python -m experiments.phase1_kuhn_vanilla
    uv run python -m experiments.phase1_kuhn_vanilla iterations=1000  # smoke
...
def should_log(iter_idx: int, log_every: int, dense_prefix: int) -> bool:
    ...
    under a pure ``log_every=100`` cadence.
    """
...
def _run_training(cfg: DictConfig) -> dict[str, np.ndarray | float | int | bool]:
    """Train Vanilla CFR on Kuhn and return the convergence history."""
    run_name = f"kuhn-vanilla-seed{cfg.seed}"
    run = wandb.init(
    ...
    for t in range(1, int(cfg.iterations) + 1):
        ...
        expl_mbb_val = exploitability_mbb(game, avg_strategy)
    ...
    # O(1/√T) sanity snapshots — 3 data points on the log-log curve.
    steps_arr = np.asarray(steps)
    mbb_arr = np.asarray(expl_mbb)
    log.info("O(1/sqrt(T)) snapshots:")
    for snap in (100, 1000, int(cfg.iterations)):
        hits = np.where(steps_arr == snap)[0]
        if hits.size:
            idx = int(hits[0])
            log.info(
                "  iter=%5d  expl=%.4f mbb/g  game_value=%+.6f",
...
def _plot_convergence(
    ...
    ax_b.set_title("Phase 1 Exit Criterion tracking")
    ax_b.grid(True, alpha=0.3)
    ax_b.legend(loc="upper right", fontsize=8)

    fig.suptitle(
        f"Kuhn Vanilla CFR — seed={cfg.seed}  iters={cfg.iterations}",
        y=1.02,
    )
    fig.tight_layout()
...
@hydra.main(version_base="1.3", config_path="conf", config_name="phase1_kuhn")
